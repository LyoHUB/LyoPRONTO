...
def execute_simulation(config):
    """
    Run the selected simulation tool with the provided configuration.
    Returns output data based on the chosen simulation mode.
    """
    sim_type = config['sim']['tool']
    output_data = None
    ...
    if sim_type == 'Freezing Calculator':
        ...
        if config['sim']['Kv_known'] and config['sim']['Rp_known']:
        ...
        elif not config['sim']['Kv_known'] and config['sim']['Rp_known']:
            output_data = _optimize_kv_parameter(config)
        elif config['sim']['Kv_known'] and not config['sim']['Rp_known']:
            output_data = _optimize_rp_parameter(config)
        else:
            raise ValueError("With the current implementation, either Kv or Rp must be specified.")
        ...
        output_data = _run_optimizer(config)
...
def _run_optimizer(config):
    """Helper to run optimization with variable parameters."""
    args = ( config['vial'], config['product'], config['ht'],
            config['Pchamber'], config['Tshelf'], config['dt'],
            config['eq_cap'], config['nVial'])
    if config['sim']['Variable_Pch'] and config['sim']['Variable_Tsh']:
        return opt_Pch_Tsh.dry(*args)
    elif config['sim']['Variable_Pch']:
        return opt_Pch.dry(*args)
    elif config['sim']['Variable_Tsh']:
        return opt_Tsh.dry(*args)


def save_configuration_legacy(config, timestamp):
    """
    Persist configuration parameters to a CSV file with timestamped filename.
    """
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        if sim['tool'] == 'Freezing Calculator':
            writer.writerow(['h_freezing [W/m^2/K]:',config['h_freezing']])
        elif sim['Kv_known']:
            writer.writerow(['KC [cal/s/K/cm^2]:',config['ht']['KC']])
            writer.writerow(['KP [cal/s/K/cm^2/Torr]:',config['ht']['KP']])
            writer.writerow(['KD [1/Torr]:',config['ht']['KD']])
        elif not sim['Kv_known']:
            writer.writerow(['Kv range [cal/s/K/cm^2]:',config['Kv_range'][:]])
            writer.writerow(['Experimental drying time [hr]:',config['t_dry_exp']])


        if sim['tool'] == 'Freezing Calculator':
            0
        elif sim['tool'] == 'Design-Space-Generator':
            writer.writerow(['Chamber pressure set points [Torr]:',config['Pchamber']['setpt'][:]])
        elif not(sim['tool'] == 'Optimizer' and sim['Variable_Pch'] == 'Y'):
            for i in range(len(config['Pchamber']['setpt'])):
                writer.writerow(['Chamber pressure setpoint [Torr]:',config['Pchamber']['setpt'][i],'Duration [min]:',config['Pchamber']['dt_setpt'][i]])
            writer.writerow(['Chamber pressure ramping rate [Torr/min]:',config['Pchamber']['ramp_rate']])
        else:
            writer.writerow(['Minimum chamber pressure [Torr]:',config['Pchamber']['min']])
            writer.writerow(['Maximum chamber pressure [Torr]:',config['Pchamber']['max']])
        writer.writerow([''])
        
        if sim['tool'] == 'Design-Space-Generator':
            writer.writerow(['Intial shelf temperature [C]:',config['Tshelf']['init']])
            writer.writerow(['Shelf temperature set points [C]:',config['Tshelf']['setpt'][:]])
            writer.writerow(['Shelf temperature ramping rate [C/min]:',config['Tshelf']['ramp_rate']])
        elif not(sim['tool'] == 'Optimizer' and sim['Variable_Tsh'] == 'Y'):
            for i in range(len(config['Tshelf']['setpt'])):
                writer.writerow(['Shelf temperature setpoint [C]:',config['Tshelf']['setpt'][i],'Duration [min]:',config['Tshelf']['dt_setpt'][i]])
            writer.writerow(['Shelf temperature ramping rate [C/min]:',config['Tshelf']['ramp_rate']])
        else:
            writer.writerow(['Minimum shelf temperature [C]:',config['Tshelf']['min']])
            writer.writerow(['Maximum shelf temperature [C]:',config['Tshelf']['max']])
        
        writer.writerow(['Time Step [hr]', config['dt']])
        writer.writerow(['Equipment Parameter a [kg/hr]', config['eq_cap']['a']])
        writer.writerow(['Equipment Parameter b [kg/hr/Torr]', config['eq_cap']['b']])
        writer.writerow(['Number of Vials', config['nVial']])

def save_csv(output_data, config, timestamp):
    """
    ...
    if sim['tool'] == 'Freezing Calculator':
        ...
        Tshelf = config['Tshelf']
        Pchamber = config['Pchamber']
        ...
        try:
            ...
            for i in range(np.size(Tshelf['setpt'])):
                ...
                writer.writerow(['Product Temperature = ',str(config['product']['T_pr_crit'])])
...
def generate_visualizations(output_data, config, timestamp):
    """
    ...
    if config['sim']['tool'] == 'Freezing Calculator':
        _plot_freezing_results(output_data, figure_props, timestamp)
    elif config['sim']['tool'] in ['Primary Drying Calculator', 'Optimizer']:
        _plot_drying_results(output_data, figure_props, timestamp)
    elif config['sim']['tool'] == 'Design-Space-Generator':
        _plot_design_space(output_data, config, figure_props, timestamp)
...
def _plot_freezing_results(data, props, timestamp):
    ...
    ax.plot(data[:, 0], data[:, 1], '-k', linewidth=props['linewidth'], label='Shelf Temperature')
    ax.plot(data[:, 0], data[:, 2], '-b', linewidth=props['linewidth'], label='Product Temperature')
    ax.set_xlabel('Time [hr]', fontsize=props['axis_fontsize'], fontweight='bold')
    ax.set_ylabel('Temperature [°C]', fontsize=props['axis_fontsize'], fontweight='bold')
...
def _plot_drying_results(data, props, timestamp):
    ...
    ax2 = ax1.twinx()
    ax1.plot(data[:,0],data[:,4],'-o',color='b',markevery=5,linewidth=linewidth, markersize=marker_size, label = "Chamber Pressure")
    ax2.plot(data[:,0],data[:,5],'-',color=[0,0.7,0.3],linewidth=linewidth, label = "Sublimation Flux")
    ...
    ax.plot(data[:,0],data[:,1],'-b',linewidth=linewidth, label = "Sublimation Front Temperature")
    ax.plot(data[:,0],data[:,2],'-r',linewidth=linewidth, label = "Maximum Product Temperature")
    ax.plot(data[:,0],data[:,3],'-o',color='k',markevery=5,linewidth=linewidth, markersize=marker_size, label = "Shelf Temperature")
...
def _plot_design_space(data, config, props, timestamp):
    ...
    ax.plot([Pchamber['setpt'][0]*constant.Torr_to_mTorr,Pchamber['setpt'][-1]*constant.Torr_to_mTorr],ds_pr[3,:],'-o',color='r',linewidth=lineWidth, label = ("T$_{pr}$ = "+str(config['product']['T_pr_crit'])+"°C"))
    for i in range(np.size(Tshelf['setpt'])):
